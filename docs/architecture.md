...
[x] GET    /api/sessions                    → list all sessions for current user (Sprint 6)
...
[x] GET    /api/sessions/{id}/tailoring-jobs → batch status dashboard (Sprint 6)
...
[x] GET    /api/jds/{id}/tailoring          → per-JD tailoring history (Sprint 6)
...
[x] GET    /api/jds/{id}/tailoring/{job_id}/package → download zip bundle (ADR-014, Sprint 11)
```
