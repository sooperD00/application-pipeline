...
**Cookie auth shipped.** Replaced the `get_current_user` stub (select first user from DB) with cookie-based anonymous sessions. Each browser gets a unique `auth_token` on first visit → own User row → fully isolated data. `secure` flag conditional on `ENVIRONMENT` setting so cookies work on both localhost and Railway HTTPS. Phase 1 adds Google sign-in that adopts the anonymous user's data ([s-26220f]).
...
## Sprint 4 - Claude batch analysis service + SSE (~4–5 hours) 3/4/2026 3:05p-5:03p
